...
export async function searchPosts(query: string): Promise<BlogPost[]> {
  try {
    ...
    const filtered = posts.filter((post) => {
      ...
      const region = (post.metadata.region?.value || '').toLowerCase();
      ...
      const categories = (post.metadata.categories || []).map(
        (c) => (c.metadata?.name || c.title || '').toLowerCase()
      );
      ...
      return (
        ...
        categories.some((c) => c.includes(lowerQuery))
      );
    });
